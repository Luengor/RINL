...
color principal es el naranja, que se utiliza para los botones, gráficos y
...
para que no distraiga al usuario, pero está presente para evitar una percepción
de vacío o que la aplicación está incompleta.}
...
Un aspecto muy importante en la rama de la interacción persona ordenador es cómo
el usuario interactúa con el sistema. Puesto que el videojuego está diseñado
para ser utilizado con un dispositivo de captura de movimiento lejos de la
pantalla y, por tanto, del teclado y el ratón, la interacción se realiza
directamente con el cuerpo del usuario.

En este caso, siempre que es necesario interaccionar de alguna manera con el
sistema, se presenta el avatar del usuario en la pantalla, que se mueve de la
misma manera que el usuario, similar a un espejo. Esto permite que el usuario
pueda ver cómo se mueve su avatar y, por tanto, cómo se está interaccionando con
el sistema. Cualquier acción que se deba realizar se puede realizar con dicho
avatar.

Por ejemplo, en el videojuego, cuando se presentan varias opciones a elegir en
un menú, el usuario selecciona las opciones moviendo cualquiera de sus manos
sobre el botón correspondiente y manteniéndola ahí durante un breve periodo de
tiempo\footnotemark. Hacer que el usuario utilice su cuerpo para interactuar con
el sistema completo es una forma de eliminar la fricción que puede existir entre
el usuario y la aplicación, ya que no es necesario acercarse a la pantalla para
interactuar con ella, ni utilizar un teclado o un ratón.

\footnotetext{También se da la opción de hacer clic con un ratón convencional,
pero no es la forma recomendada de interactuar con el sistema.}
...
se han centrado en observar cómo usuarios reales interactúan con la interfaz por
sí mismos y recoger su retroalimentación sobre la usabilidad y su experiencia

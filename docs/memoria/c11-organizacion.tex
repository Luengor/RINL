...
través del \gls{ORM} SQLAlchemy y se alojan en la base de datos relacional \texttt{PostgreSQL}.

\subsubsection{\Gls{backend}}
El \gls{backend} del sistema se encarga de implementar la lógica de negocio y de proporcionar una
\gls{api} para acceder a los datos de la aplicación. Está desarrollada en \texttt{Python} utilizando
el \gls{framework} \texttt{FastAPI}. Se divide en tres partes fundamentales:
\begin{itemize}
...
específico. Por ejemplo, la clase \texttt{UserRouter} se encarga de gestionar las rutas relacionadas
con los usuarios, mientras que \texttt{ShapeDAO} se encarga de proveer acceso a los datos relacionados
con las formas físicas.

\subsubsection{\Gls{frontend}}
El \gls{frontend} está desarrollado en \texttt{React} y se encarga de proporcionar la interfaz de
...
las diferencia es que la primera de ellas ha sido autogenerada a partir de la especiciación OpenAPI
del \gls{backend}, mientras que la segunda ha sido desarrollada a mano para adaptarse a las
necesidades del videojuego.

Por último, el \gls{frontend} también se encarga de realizar la detección de posturas a través de
la cámara del dispositivo del usuario. Para ello, se utiliza la biblioteca \texttt{MediaPipe}, que
...
El videojuego desarrollado en Unity es un componente embebido en la aplicación web que se comunica
con esta a través de eventos de JavaScript. A excepción de la situación del videojuego una vez
se ha exportado, este no tiene ninguna característica destacable, ya que se trata de un videojuego
simple que no interacciona mucho con el resto del sistema.

En cuanto a la conexión que mantiene con el \gls{frontend}, esta se realiza a través de una clase
singleton, que se encarga de gestionar la comunicación entre el videojuego y el \gls{frontend} y
de mantener los datos obtenidos (como la postura detectada) accesible para todos los objetos de la
escena.
...
editor de \texttt{Unity} como en la aplicación web una vez exportada. Estas pruebas de rendimiento
...
Unity mencionado anteriormente. Durante el desarrollo del proyecto se ha utilizado la herrmienta de
...
Por último, los recursos personales utilizados me incluirían a mí y a los tutores del proyecto. Todas las
